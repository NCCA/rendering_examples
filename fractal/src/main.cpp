...
void key_callback(GLFWwindow* window, int key, int /*scancode*/, int action, int mods)
{
    ...
    if (action == GLFW_PRESS) {
        ...
        case GLFW_KEY_SPACE:
            ...
            break;
        case GLFW_KEY_1: //exit the application
            g_scene.setCurrentFractal(FractalScene::FRACTAL_JULIA);
            break;
        case GLFW_KEY_2: //exit the application
            g_scene.setCurrentFractal(FractalScene::FRACTAL_MANDELBROT);
            break;
        case GLFW_KEY_T:
            g_scene.setRenderObject(FractalScene::OBJECT_TEAPOT);
            break;
        case GLFW_KEY_P:
            g_scene.setRenderObject(FractalScene::OBJECT_PLANE);
            break;
        }
    }
...
//#endif
    ...
    std::cout << "******************* USAGE **************************\n"
              << "1: Use Julia Fractal\n"
              << "2: Use Mandelbrot Fractal\n"
              << "t: Render on a teapot\n"
              << "p: Render on a screen aligned plane\n"
              << "<SPACE>: Toggle animating\n"              
              << "<ESC>: Quit\n"
              << "****************************************************\n";           
